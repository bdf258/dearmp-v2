-- ============================================================================
-- FIX TEST EMAIL RPC FUNCTIONS
-- Update functions to accept office_id as parameter since server uses
-- service role key (not user JWT), and auth.uid() returns NULL in that context.
-- ============================================================================

-- Drop existing functions first to handle signature changes
DROP FUNCTION IF EXISTS insert_legacy_test_email(INTEGER, TEXT, TEXT, TEXT, JSONB, JSONB, JSONB, TIMESTAMPTZ);
DROP FUNCTION IF EXISTS get_legacy_test_emails(INTEGER);
DROP FUNCTION IF EXISTS delete_legacy_test_email(UUID);
...
-- This creates an overloaded function with the new signature
-- NOTE: Accepts office_id as parameter since server uses service role key
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION insert_legacy_test_email(UUID, INTEGER, TEXT, TEXT, TEXT, JSONB, JSONB, JSONB, TIMESTAMPTZ) TO authenticated;
GRANT EXECUTE ON FUNCTION insert_legacy_test_email(UUID, INTEGER, TEXT, TEXT, TEXT, JSONB, JSONB, JSONB, TIMESTAMPTZ) TO service_role;
GRANT EXECUTE ON FUNCTION get_legacy_test_emails(UUID, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION get_legacy_test_emails(UUID, INTEGER) TO service_role;
GRANT EXECUTE ON FUNCTION delete_legacy_test_email(UUID, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION delete_legacy_test_email(UUID, UUID) TO service_role;
GRANT EXECUTE ON FUNCTION get_legacy_email_details(UUID, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION get_legacy_email_details(UUID, UUID) TO service_role;

-- Comments
COMMENT ON FUNCTION insert_legacy_test_email(UUID, INTEGER, TEXT, TEXT, TEXT, JSONB, JSONB, JSONB, TIMESTAMPTZ) IS 'Insert a test email into the legacy.emails table for testing the triage pipeline (server version with explicit office_id)';
COMMENT ON FUNCTION get_legacy_test_emails(UUID, INTEGER) IS 'List test emails for the specified office (server version with explicit office_id)';
COMMENT ON FUNCTION delete_legacy_test_email(UUID, UUID) IS 'Delete a test email (server version with explicit office_id)';
